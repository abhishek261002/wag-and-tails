...
export default function PartnerCheckoutScreen() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      wagApi.client.get<any>('/users/me'),
    ]).then(([c, u]) => {
      setCart(c);
      setDiscount(Number((c as any)?.discount ?? 0));
  ...
  const placeOrder = async () => {
    ...
    setPlacing(true);
    try {
      const order = await wagApi.store.checkout(selectedAddressId, paymentMethod, couponCode || undefined);
