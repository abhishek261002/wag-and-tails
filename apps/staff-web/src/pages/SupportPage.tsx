...
export default function SupportPage() {
  ...
  const loadTickets = useCallback(() => {
    wagApi.support.listTickets({ status }).then(setTickets).catch(() => {});
  }, [status]);
  ...
  const openTicket = (t: SupportTicket) => setSelected(t);
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      toast.toast({ type: 'error', title: 'Could not send reply', message: err?.message });
      setReply(content);
    } finally {
      setSending(false);
    }
  ...
  const handleEscalate = async () => {
    if (!selected) return;
    const reason = window.prompt('Why is this being escalated to the admin team?');
    if (!reason) return;
    setEscalating(true);
    try {
      const updated = await wagApi.support.escalate(selected.id, reason);
      setSelected(updated);
      loadMessages(selected.id);
      loadTickets();
      toast.toast({ type: 'success', title: 'Escalated to admin' });
    } catch (err: any) {
      toast.toast({ type: 'error', title: 'Could not escalate', message: err?.message });
    } finally {
      setEscalating(false);
    }
  };
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      setSelected(updated);
      loadTickets();
      toast.toast({ type: 'success', title: 'Ticket marked resolved' });
    } catch (err: any) {
      toast.toast({ type: 'error', title: 'Could not update ticket', message: err?.message });
    }
  };
      ...
      <div className="flex gap-5 flex-1 min-h-0">
        ...
        <div className="w-[340px] shrink-0 flex flex-col bg-white rounded-2xl border border-[#E8D8CC] overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {tickets.length === 0 ? (
              <div className="p-6 text-center text-[#9E7B6A] text-sm">No {status.replace('_', ' ')} tickets</div>
            ) : (
              tickets.map((t) => (
                <button
                  key={t.id}
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {t.escalated && <Badge variant="error">Escalated</Badge>}
                  </div>
                  <div className="text-xs text-[#9E7B6A] mt-1">
                    {t.user?.profile ? `${t.user.profile.firstName} ${t.user.profile.lastName}` : 'User'} · {formatDistanceToNow(new Date(t.createdAt), { addSuffix: true })}
                  </div>
                </button>
              ))
            )}
        ...
        <div className="flex-1 flex flex-col bg-white rounded-2xl border border-[#E8D8CC] overflow-hidden min-w-0">
          {!selected ? (
            <div className="flex-1 flex items-center justify-center text-[#9E7B6A] text-sm">Select a ticket to view the conversation</div>
          ) : (
            <>
